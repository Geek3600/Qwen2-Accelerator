...
//
//
//  val QCache = Module(new R1W1Mem(LBATCHSIZE * MAX_PREFILL, HEAD_VECNUM * DATAW)) //對於當前的情況，這個是可以勝率
...
//  QCache.io.waddr := Mux(is_prefill, lbatch_cnt * MAX_PREFILL.U + prefill_cnt, lbatch_cnt)
//
...
//  QCache.io.raddr := Mux(is_prefill, lbatch_cnt_r * MAX_PREFILL.U + comp_prefill_q_cnt, lbatch_cnt_r)
//
