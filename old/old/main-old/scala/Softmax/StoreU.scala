...
class StoreU extends Module {
  ...
  val out_valid = io.data_in_valid && (~Mux(is_prefill, prefill_batchsize_last, decode_batchsize_last))
  val out_last = Mux(is_prefill, prefill_batchsize_last, decode_batchsize_last)

  io.data_out := io.data_in
  io.data_out_valid := out_valid
  io.data_out_addr := RegNext(out_addr)
  io.data_out_last := RegNext(out_last, false.B)
//   printf(p"[StoreU] data_out_valid = ${io.data_out_valid}, decode_batchsize_cnt = $decode_batchsize_cnt, out_addr = $out_addr, data_out = ${Binary(io.data_out)}\n \n")
}
