...
class LoadU extends Module {
  ...
  val vector_decode_plus = math.max(vector_num, MAX_SEQLEN*HEAD_VECNUM/MULNUM)
  // 为什么在 vector_cnt 层空转：因为 Decode 模式下，每个 batch 只有一个 Q，LoadU 发完 32 周期数据后，需要等 DM 完成计算才能处理下一个 batch。
  val vector_cnt = Wire(UInt(log2Up(vector_decode_plus).W))
  val vector_last =vector_cnt ===  Mux(is_prefill, 
      (vector_num - 1).U , // prefill 32
      (vector_decode_plus - 1).U // decode 52 //TODO: 感觉这个52不太对
    )
