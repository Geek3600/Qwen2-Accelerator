...
// 流水级1: LayerNorm - 对输入进行层归一化
  ...
  val cu_inst = Module(new LayerNormCU)
  ...
  cu_inst.io.w_data := io.w_in
  cu_inst.io.w_valid := io.w_valid
...
object LayerNormGen extends App {
  emitVerilog(new LayerNorm, Array("--target-dir", "generated"))
}