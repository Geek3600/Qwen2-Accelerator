...
class CU extends Module {
  ...
  val weight_w_cnt = Wire(UInt(log2Up(ROW + 1).W))
  ...
  weight_w_cnt := RegEnable(
    Mux(weight_w_last, 0.U, weight_w_cnt + 1.U),
    0.U,
    io.w_valid
  )
  ...
  val mul_list = List.fill(ROW)(List.fill(COL)(Module(new Muler)))
  for (i <- 0 until ROW) {
    for (j <- 0 until COL) {
  ...
  val buzy_mux = Mux(
    res_done,
    Mux(send_st, buzy, idle),
    buzy
  )
  state := Mux(is_idle, idle_mux, buzy_mux)

  res_batchsize_cnt := RegEnable(
    Mux(res_batchsize_last, 0.U, res_batchsize_cnt + 1.U),
    0.U,
    is_buzy
  )

  res_vector_cnt := RegEnable(
    Mux(res_vector_last, 0.U, res_vector_cnt + 1.U),
    0.U,
    is_buzy && res_batchsize_last
  )

  val psum0_token = MuxLookup(res_batchsize_cnt, 0.U)(
    (0 until BATCHSIZE).map { i => i.U -> Cat(psums0(i).reverse) }
  )

  val psum1_token = MuxLookup(res_batchsize_cnt, 0.U)(
    (0 until BATCHSIZE).map { i => i.U -> Cat(psums1(i).reverse) }
  )
  ...
  val psum_res = MuxLookup(RegNext(res_vector_cnt), psum_batch(MEM_WIDTH - 1, 0))(
    (0 until res_vector_num).map { i => i.U -> psum_batch((i + 1) * MEM_WIDTH - 1, i * MEM_WIDTH) }
  )
