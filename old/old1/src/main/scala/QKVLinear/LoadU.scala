...
class LoadU extends Module {
  ...
  // 计数器声明 (必须在状态机之前，因为状态机引用了 *_last 信号)
  // 内层：遍历 N 个 token (0..N-1)
  ...
  // 循环结构详解
  // for block_cnt in :                 // 外层：权重的列分块
  //   for vector_cnt 0..63 in 0..63:   // 中层：输入的行分块
  //     for batchsize_cnt in 0..N-1:   // 内层：token遍历
  //       读取 DataMen[vector_cnt + batchsize_cnt * 64]
  // 地址生成（请注意！虽然batchsize_cnt是最内层，但是由于乘法阵列的设计原因，输入数据是需要跨行读取的）
