...
class QKVLinear extends Module {
  ...
  val collect_cnt = Wire(UInt(log2Up(COLLECT_NUM + 1).W))
  val collect_last = collect_cnt === (COLLECT_NUM - 1).U

  collect_cnt := RegEnable(
    Mux(collect_last, 0.U, collect_cnt + 1.U),
    0.U,
    su_inst.io.data_out_valid && is_collecting
  )

  // 缓存 2304 维向量
  val vec_buffer = Reg(Vec(COLLECT_NUM, UInt(MEM_WIDTH.W)))
  when(su_inst.io.data_out_valid && is_collecting) {
    vec_buffer(collect_cnt) := su_inst.io.data_out
  }

  // 将 2304 维向量重组为 8-bit 元素数组
  val full_vec = vec_buffer.asUInt.asTypeOf(Vec(OUT_VECTOR, UInt(DATAW.W)))
  ...
  val all_output_done = output_last && token_last && head_last
  ...
  switch(state) {
    ...
    is(collecting) {
      when(collect_last && su_inst.io.data_out_valid) {
        state := outputting
      }
    }
    is(outputting) {
