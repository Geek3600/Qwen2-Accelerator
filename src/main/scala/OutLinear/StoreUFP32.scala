...
class StoreUnitFP32 extends Module {
  ...
  subvector_cnt := RegEnable(Mux(subvector_last, 0.U, subvector_cnt + 1.U), 0.U, io.data_in_valid)

  val batchsize_last = batchsize_cnt === actual_batchsize
  batchsize_cnt := RegEnable(Mux(batchsize_last, 0.U, batchsize_cnt + 1.U), 0.U, io.data_in_valid && subvector_last)

  val block_last = block_cnt === (COLBLOCK - 1).U
  block_cnt := RegEnable(Mux(block_last, 0.U, block_cnt + 1.U), 0.U, io.data_in_valid && subvector_last && batchsize_last)
