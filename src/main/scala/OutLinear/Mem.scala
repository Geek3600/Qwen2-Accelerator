...
class DataMem(depth: Int, width: Int) extends Module {
  ...
  // r_sel points at the next buffer to consume; when false we should first
  // read the buffer that was filled first (mem0), then flip to mem1.
  val mem0_r = mem0.read(io.r_addr, !r_sel && io.r_en)
  val mem1_r = mem1.read(io.r_addr, r_sel && io.r_en)
